        ...
        Vector3 GetForwardMovement(out float speed)
        {
            ...
            if (m_MovementInputType == MovementInputType.AnalogAutoRun)
            {
                if (fwd > Mathf.Epsilon)
                    speed = Mathf.Lerp(m_MovementSpeed * absFwd, m_MovementRunSpeed * absFwd, absFwd);
            ...
            else
            {
                if (fwd > Mathf.Epsilon)
                {
                    speed = m_Running ? m_MovementRunSpeed : m_MovementSpeed * (m_MovementInputType == MovementInputType.Analog ? absFwd : 1f);
                }
                else if (fwd < -Mathf.Epsilon)
                {
